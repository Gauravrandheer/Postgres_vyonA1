...
const generateData = (batchSize,startIndex) => {
  ...
  for (let i = startIndex; i < startIndex + batchSize; i++) {
    const originalUrl = `https://hundred.com/page/${i}`;
    const shortCode = `hundred${i}`;
...
const insertData = async () => {
  ...
  try {
    ...
    const totalRows = 100000000;  // Total rows to insert
    const queryText = `
    ...
    console.log(`100 million rows inserted in ${elapsedTime} seconds.`);
